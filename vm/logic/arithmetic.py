...
def add(computation: ComputationAPI) -> None:
    ...
    result = (left + right) & constant.UINT_256_MAX
...
def smod(computation: ComputationAPI) -> None:
    ...
    if mod == 0:
        ...
        result = (abs(value) % abs(mod) * pos_or_neg) & constant.UINT_256_MAX
...
def div(computation: ComputationAPI) -> None:
    ...
    if denominator == 0:
        ...
        result = (numerator // denominator) & constant.UINT_256_MAX
...
def exp(computation: ComputationAPI, gas_per_byte: int) -> None:
    ...
    if exponent == 0:
        ...
        result = pow(base, exponent, constant.UINT_256_CEILING)
...
def signextend(computation: ComputationAPI) -> None:
    ...
    if bits <= 31:
        ...
        sign_bit = 1 << testbit
        if value & sign_bit:
            result = value | (constant.UINT_256_CEILING - sign_bit)
        else:
...
def shl(computation: ComputationAPI) -> None:
    ...
    if shift_length >= 256:
        ...
        result = (value << shift_length) & constant.UINT_256_MAX
...
def sar(computation: ComputationAPI) -> None:
    ...
    if shift_length >= 256:
        result = 0 if value >= 0 else constant.UINT_255_NEGATIVE_ONE
    else:
        result = (value >> shift_length) & constant.UINT_256_MAX
